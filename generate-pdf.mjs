...
function createManualPDF(mdContent, title, subtitle, outputPath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith("|")) {
      ...
      // Parse table - filter out separator row
...
createManualPDF(
  ...
  resolve(root, "MANUAL-USUARIO.pdf")
);
